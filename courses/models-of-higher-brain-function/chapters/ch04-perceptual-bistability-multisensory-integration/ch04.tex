...
\subsection{Linking the model to behavior}

The slides connect the model to psychophysical reports. In a typical task, an observer judges whether a comparison stimulus is wider or taller than a reference. The response curve gives the probability of saying ``yes'' as a function of the comparison value. The point of subjective equality estimates the perceived value, and the slope of the curve estimates discrimination precision.

The Ernst and Banks visual--haptic experiment manipulates visual noise while keeping haptic information available. The maximum-likelihood model predicts:
\begin{itemize}
    \item when visual noise is low, visual precision is high and the visual cue gets more weight;
    \item when visual noise is high, visual precision is low and the haptic cue gets more weight;
    \item the bimodal threshold should be lower than the unimodal thresholds because the combined estimate has lower variance.
\end{itemize}

The behavioral result is important because it is not only a qualitative story about ``using both senses.'' The experiment estimates unimodal variances, predicts the bimodal weights and thresholds, and then compares those predictions with human responses.
...
\begin{itemize}
    ...
    \item M. O. Ernst and M. S. Banks (2002), human visual--haptic cue combination consistent with maximum-likelihood integration.
\end{itemize}
