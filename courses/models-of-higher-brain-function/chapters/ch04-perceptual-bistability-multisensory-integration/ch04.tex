...
The problem solved by a bistability model is persistence without commitment. The sensory input does not change, but perception must occupy one interpretation at a time and still remain able to switch when the current interpretation becomes unstable.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.96\textwidth]{figures/mhbf-lecture3-bistability-mechanisms.png}
\caption{Four linked mechanisms for bistable perception. Competition prevents both interpretations from dominating together, adaptation weakens the active interpretation, noise can push the system across an attractor barrier, and a sampling view interprets switches as visits to different modes of a posterior over percepts.}
\end{figure}

\subsection{A two-population competition model}

The problem now is to turn the qualitative word ``competition'' into state variables that can be simulated, fit, and related to neural activity. A minimal rate model uses two activity variables, \(u_1(t)\) and \(u_2(t)\), for the two perceptual populations:
\[
\tau_u \frac{d u_1}{dt}
=
-u_1
...
\begin{tikzpicture}[>=Latex, thick]
    ...
    \node[align=center] at (0,-2.95) {self-excitation stabilizes each percept; mutual inhibition prevents joint dominance};
\end{tikzpicture}
...
To analyze a bistable-perception model, first identify which variables are fast activities, which variables are slow fatigue or synaptic-depression states, and which parameters change the stability of the mixed state. The measurable outputs are dominance fractions, switch rates, and the distribution of dominance durations, not just the existence of two fixed points.

\subsection{Mechanism 1: adaptation as a deterministic switch}

The problem adaptation solves is the otherwise unexplained end of a stable percept. If competition and self-excitation alone make one state persistent, some slow negative feedback is needed to make the currently dominant state lose its advantage. Adaptation means that a strongly active population becomes less effective over time. This can be implemented as neuronal fatigue or synaptic depression. A simple adaptation equation is
\[
\tau_a \frac{d a_i}{dt}
=
-a_i + \gamma u_i,
...
\paragraph{Practical implication.}
If the observed switches are nearly periodic, a deterministic adaptation oscillator may be sufficient. If the switch times are broadly distributed, adaptation should be treated as a changing hazard or barrier height rather than as a clock that directly triggers every transition.

\subsection{Percept durations and the gamma distribution}

The problem solved by duration statistics is model discrimination. A model can produce perceptual alternations but still fail if it predicts the wrong shape of dominance times. The lecture emphasizes that percept durations are often fit by a gamma distribution:
\[
p(T)
=
\frac{1}{\tau^k \Gamma(k)}
...
The problem noise solves is trial-to-trial irregularity. Neural activity fluctuates. Even if percept \(A\) is currently dominant, a fluctuation can make \(A\) temporarily less suppressive or make \(B\) temporarily stronger. If the fluctuation is large enough, the system crosses the boundary between attractors.
...
\paragraph{Practical implication.}
When comparing adaptation-only, noise-only, and hybrid models, compute both the mean dominance time and the full distribution. The hybrid account is attractive because it predicts a changing escape probability: early after a switch the active attractor is protected, later it is easier for noise to carry the state across the barrier.

\subsection{Bayesian sampling interpretation}

The problem solved by the Bayesian sampling view is how to read perceptual alternation as inference rather than as a defect. Let \(h\) denote a possible perceptual interpretation and \(s\) the sensory data. The posterior distribution is
\[
p(h\mid s)
\propto
p(s\mid h)p(h).
...
The second half of the lecture turns to a different ambiguity: different sensory modalities provide different noisy measurements of the same world property. For example, vision and touch can both provide information about object size. The problem is to infer the hidden world state \(h\) from sensory observations \(s_1\) and \(s_2\), while respecting that some cues are more reliable than others.

The mental model is a measurement device with multiple sensors. Each cue gives a noisy measurement of the same latent world variable; inference should let precise sensors pull the estimate more strongly and should reduce uncertainty when the cues are compatible.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.96\textwidth]{figures/mhbf-lecture3-cue-integration.png}
\caption{Cue integration as a sequence of inference checks. A hidden state causes modality-specific cues, reliable cues pull the fused estimate more strongly, psychometric PSEs and thresholds reveal the weights and precision, and a causal-inference gate determines whether the cues should be fused or kept separate.}
\end{figure}

\subsection{Maximum-likelihood cue combination}

The problem solved by maximum-likelihood cue combination is how to make the word ``reliability'' quantitative. The lecture derives the estimate under three assumptions:
\begin{itemize}
...
\paragraph{Practical implication.}
Given unimodal thresholds, convert each threshold into a variance or precision, predict the bimodal weight and threshold, and compare those predictions with the observed PSE shift and discrimination slope. This turns a qualitative statement about integration into a falsifiable residual check.
