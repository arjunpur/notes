...
\title{Acquisition and Analysis of Neural Data -- Summer\\Lecture Notes}
\author{Arjun Puri}
\date{Summer 2026}
...
This distinction is foundational for the rest of the course. Lectures 1--2 focus mainly on spike-train representations, rate estimation, and encoding. Lecture 3 turns those tools toward spike-train variability and correlations, which are the first practical route to classifying neurons and inferring functional relationships between recorded cells.
...
\subsubsection{Interpreting the trial-averaged rate}
...
\subsubsection{Detour: why the averaged spike train becomes the averaged rate inside the integral}
...
\clearpage
\section{Lecture 3: Spike-train variability, Poisson models, and correlations}

Lecture 3 continues the spike-train statistics developed at the end of Lecture 2. The practical motivation is experimental: modern probes such as Neuropixels can record from hundreds of channels at once, so one wants statistics that help identify neurons, classify cell types, and detect possible functional connections from spike trains recorded \emph{in vivo}. The lecture treats the homogeneous and inhomogeneous Poisson processes as baseline models, then asks which empirical summaries reveal departures from that baseline.

The main conceptual move is from first-order firing rates to second-order temporal structure. A firing rate tells us how likely spikes are as a function of time. It does not tell us whether spikes are regular, bursty, oscillatory, refractory, or coordinated across neurons. Count statistics, interspike-interval statistics, autocorrelations, and cross-correlations are the workhorse tools that start to answer those questions.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/aand-lecture3-spike-statistics-overview.png}
\caption{Four complementary summaries of a spike raster. Spike counts measure trial-to-trial variability, ISI statistics measure successive waiting times, autocorrelograms compare all pairs of spikes within one train, and cross-correlograms compare lagged spike pairs across two trains.}
\end{figure}

\subsection{Spike-count variability}

Suppose the spike count in a fixed observation window of duration $T$ is the random variable $n$. Its distribution is denoted
\[
p_n(k)=\Pr[n=k], \qquad k=0,1,2,\dots .
\]
The mean count and variance are
\[
\langle n\rangle=\sum_{k=0}^{\infty} k\,p_n(k),
\qquad
\sigma_n^2=\langle n^2\rangle-\langle n\rangle^2.
\]
The dimensionless comparison emphasized in the lecture is the \emph{Fano factor}
\[
F:=\frac{\sigma_n^2}{\langle n\rangle}.
\]
The point of $F$ is not that every neuron should be Poisson. The point is that the homogeneous Poisson process supplies a simple reference scale:
\[
P_T[n=k]=\frac{(rT)^k}{k!}e^{-rT},
\qquad
\langle n\rangle=rT,
\qquad
\sigma_n^2=rT,
...
F=1.
\]
Thus $F>1$ indicates count variability larger than the Poisson prediction, often described as overdispersion or burstiness. Conversely, $F<1$ indicates counts that are more regular than independent Poisson spiking would predict. The MT-neuron example in the slides showed that, for $T=256\,\mathrm{ms}$ windows, many neurons lie close enough to the Poisson line that Poisson statistics are a useful first approximation, even though individual neurons may deviate substantially.

\subsection{Interspike intervals and the coefficient of variation}

Count statistics ignore where the spikes occur inside the window. A complementary statistic looks at successive spike times. If
\[
0\le t_1<t_2<\cdots<t_n\le T,
\]
then the interspike intervals are
\[
\tau_i=t_{i+1}-t_i.
\]
The ISI density is defined by the small-bin limit
\[
p_{\mathrm{ISI}}(\tau)
=
\lim_{\Delta t\to 0}
\frac{\Pr[\tau\le t_{i+1}-t_i<\tau+\Delta t]}{\Delta t}.
\]
From this density one computes
\[
\langle \tau\rangle=\int_0^\infty \tau\,p_{\mathrm{ISI}}(\tau)\,d\tau,
\]
\[
\sigma_\tau^2
=
\int_0^\infty \tau^2p_{\mathrm{ISI}}(\tau)\,d\tau
-\langle \tau\rangle^2,
\qquad
\mathrm{CV}:=\frac{\sigma_\tau}{\langle \tau\rangle}.
\]

\subsubsection{ISI law for the homogeneous Poisson process}

For a homogeneous Poisson process with rate $r$, the probability of no spike in an interval of length $\tau$ is
\[
\Pr[\text{no spike in }[0,\tau]]=P_\tau[0]=e^{-r\tau}.
\]
The probability of one spike in the next infinitesimal interval $[\tau,\tau+\Delta t]$ is approximately $r\Delta t$. Therefore
\[
\Pr[\tau\le \tau_i<\tau+\Delta t]
\approx e^{-r\tau}\,r\Delta t,
\]
and hence
\[
p_{\mathrm{ISI}}(\tau)=r e^{-r\tau}, \qquad \tau\ge 0.
\]
...
\mathrm{CV}=1.
\]
The equality $\mathrm{CV}=1$ is the interval-statistics analog of the Fano-factor equality $F=1$. It is a diagnostic baseline, not a law of biology.

\subsubsection{What refractory periods change}

Real neurons cannot fire arbitrarily soon after a spike. If there is an absolute refractory period of length $\tau_{\mathrm{ref}}$, then
\[
p_{\mathrm{ISI}}(\tau)=0
\qquad\text{for}\qquad
0<\tau<\tau_{\mathrm{ref}}.
\]
One simple model is a shifted exponential waiting time,
\[
\tau=\tau_{\mathrm{ref}}+X,
\qquad
X\sim \mathrm{Exponential}(r).
\]
Then
\[
\langle \tau\rangle=\tau_{\mathrm{ref}}+\frac{1}{r},
\qquad
\sigma_\tau^2=\frac{1}{r^2},
\qquad
\mathrm{CV}=\frac{1/r}{\tau_{\mathrm{ref}}+1/r}<1.
\]
This explains why adding a refractory period tends to make the spike train more regular. The MT and V1 examples in the slides show precisely this pattern: the ISI histogram has a near-zero gap at short intervals, followed by an approximately exponential tail.

\subsection{Inhomogeneous Poisson spike trains}

The homogeneous Poisson process fixes a constant rate $r$. For many experiments this is too restrictive because stimuli and internal states vary over time. The corresponding time-dependent baseline is the \emph{inhomogeneous Poisson process}, with intensity
\[
\lambda(t)=\langle r(t)\rangle.
\]
For a small bin,
\[
\Pr[\text{one spike in }[t,t+\Delta t]]
=\lambda(t)\Delta t + o(\Delta t),
\]
and disjoint bins remain independent in the Poisson approximation.

For spike times $t_1,\dots,t_n$ in $[0,T]$, the spike-train density is
\[
p(t_1,\dots,t_n)
=
...
\]
The product term rewards the model for placing high intensity at observed spike times. The exponential term normalizes the distribution by penalizing the total predicted spike mass,
\[
\Lambda(T):=\int_0^T \lambda(t)\,dt.
\]
Indeed, the expected spike count is
\[
\mathbb{E}[n]=\Lambda(T),
\]
and the count distribution remains Poisson with mean $\Lambda(T)$ when one ignores the precise spike times. What changes relative to the homogeneous case is that spike times are no longer uniformly distributed across the interval; conditioned on $n$, they are distributed with density proportional to $\lambda(t)$.

This model is useful because it separates two questions:
\begin{itemize}
    \item does the time-varying rate explain the main stimulus-locked structure?
    \item after conditioning on that rate, is there still refractory, bursting, oscillatory, or pairwise structure left in the residual spike train?
\end{itemize}
The second question is where correlation functions enter.

\subsection{Spike-train correlation functions}

Let
\[
\rho(t)=\sum_i \delta(t-t_i),
\qquad
\rho'(t)=\sum_j \delta(t-t'_j)
\]
be two spike trains. For stationary responses with mean rates $\langle r\rangle$ and $\langle r'\rangle$, Lecture 3 defined the centered spike-train cross-correlation, more precisely a cross-covariance density, as
\[
Q_{\rho\rho'}(\tau)
:=
\frac{1}{T}\int_0^T
\left\langle
\bigl(\rho(t)-\langle r\rangle\bigr)
\bigl(\rho'(t+\tau)-\langle r'\rangle\bigr)
\right\rangle dt.
\]
When the rates are constant this can be written as
\[
Q_{\rho\rho'}(\tau)
=
\frac{1}{T}\int_0^T
...
\]
The centering term subtracts the coincidence rate expected merely from the two mean firing rates. The units are rate squared, because each spike train has units of inverse time.

The autocorrelation is the special case $\rho'=\rho$:
\[
Q_{\rho\rho}(\tau)
=
\frac{1}{T}\int_0^T
...
\right\rangle dt.
\]
Autocorrelation generalizes the ISI distribution. The ISI density only looks at \emph{successive} spikes. The autocorrelation counts \emph{all} spike pairs at each lag $\tau$, so it can reveal oscillations, bursts, and refractory gaps that are not captured by successive intervals alone.

\subsubsection{Symmetry of the autocorrelation}

For a single stationary spike train, the autocorrelation is even:
\[
Q_{\rho\rho}(\tau)=Q_{\rho\rho}(-\tau).
\]
The reason is the same as for ordinary signal autocorrelation. Ignoring boundary effects when $|\tau|\ll T$, the change of variables $u=t+\tau$ gives
\[
\int \rho(t)\rho(t+\tau)\,dt
=
\int \rho(u-\tau)\rho(u)\,du,
\]
which is the same pair-counting operation with the order of the two spikes reversed.

\subsection{Numerical correlograms from discretized spike trains}

In data analysis one usually computes a binned approximation. The key
conceptual step is that a spike train is not first a smooth function: it is a
sum of point masses. Binning replaces each point mass by a count in a small
time interval, so products of spike trains become products of nearby bin
counts. Divide $[0,T]$ into $M=T/\Delta t$ bins and let
\[
x_i=
\begin{cases}
1, & \text{if the spike train has a spike in bin }i,\\
0, & \text{otherwise.}
\end{cases}
\]
If the bin is short enough that multiple spikes in one bin are rare, $x_i$ is a
binary variable. More generally, $x_i$ can be an integer spike count. The
discretized response is approximately
\[
\rho_i \approx \frac{x_i}{\Delta t},
\]
because $x_i/\Delta t$ has units of spikes per second. The total spike count is
$n=\sum_i x_i$, and the empirical mean rate is
\[
\hat r=\frac{n}{T}
\approx
\frac{1}{M}\sum_{i=1}^M \frac{x_i}{\Delta t}.
\]

For a lag $\tau=m\Delta t$, the raw autocorrelation pair count at lag $m$ is
\[
N_m:=
\sum_{i\in I_m} x_i x_{i+m},
\qquad
I_m:=\{i:1\le i\le M,\;1\le i+m\le M\}.
\]
Thus $N_m$ is literally a histogram of ordered spike pairs: choose a spike in
bin $i$, look $m$ bins later, and count whether there is another spike there.
For $m<0$ the same formula counts pairs in the opposite temporal direction.
For an autocorrelogram, $N_{-m}=N_m$ after reversing the order of the same spike
pairs, up to finite-boundary effects.

The normalization follows directly from the continuous formula. Starting from
the uncentered term,
\[
\frac{1}{T}\int_0^T \rho(t)\rho(t+\tau)\,dt,
\]
replace the integral by a Riemann sum and set $\tau=m\Delta t$:
\[
\frac{1}{T}\int_0^T \rho(t)\rho(t+m\Delta t)\,dt
\approx
\frac{1}{T}\sum_{i\in I_m}
\left(\frac{x_i}{\Delta t}\right)
\left(\frac{x_{i+m}}{\Delta t}\right)\Delta t
=
\frac{N_m}{T\Delta t}.
\]
This is why the denominator contains one factor of $\Delta t$, not two. Each
product contributes $(\Delta t)^{-2}$, but the integral contributes a bin width
$\Delta t$.

The corresponding centered continuous correlation is approximated by
\[
Q_{\rho\rho}(m\Delta t)
\approx
\frac{N_m}{T\Delta t}
-\left(\frac{n}{T}\right)^2,
\qquad m\ne 0.
\]
This expression uses the recording duration $T$ in the denominator. A common
edge-corrected version divides by the amount of overlapping time,
$T-|m|\Delta t$, instead:
\[
\widehat Q_{\rho\rho}^{\,\mathrm{edge}}(m\Delta t)
=
\frac{N_m}{(T-|m|\Delta t)\Delta t}
-\hat r^2.
\]
The edge correction matters when $|m|$ is not negligible relative to $M$,
because fewer bin pairs are available near the ends of the recording. For small
lags in a long recording, the two normalizations are nearly identical.

The slide convention often starts with the raw histogram $N_m$ because it is easy to visualize. The normalization and baseline subtraction are then added when one wants a quantity comparable across neurons and recording durations.

The zero-lag autocorrelation bin is special:
\[
N_0=\sum_i x_i^2=\sum_i x_i=n.
\]
It counts each spike with itself and therefore produces a large central peak. In
the binned estimate this peak has height approximately $\hat r/\Delta t$ before
baseline subtraction, so it grows as the bin width becomes smaller. This is the
discrete trace of the Dirac delta at zero lag. The peak is mathematically
meaningful, but for visualization it is often omitted so that nonzero-lag
structure is visible.

\subsubsection{Cross-correlograms in the same notation}

For two neurons, with binned spike counts $x_i$ and $y_i$, the raw
cross-correlogram is
\[
N^{xy}_m:=
\sum_{i\in I_m}x_i y_{i+m}.
\]
The centered estimate is
\[
\widehat Q_{\rho\rho'}(m\Delta t)
\approx
\frac{N^{xy}_m}{T\Delta t}
-\hat r_x\hat r_y,
\qquad
\hat r_x=\frac{1}{T}\sum_i x_i,\quad
\hat r_y=\frac{1}{T}\sum_i y_i.
\]
With this convention, a positive-lag peak means that spikes in $y$ occur more
often than baseline after spikes in $x$. A negative-lag peak means that $y$
tends to precede $x$. Unlike an autocorrelogram, a cross-correlogram need not be
even: generally $N^{xy}_m=N^{yx}_{-m}$, not $N^{xy}_m=N^{xy}_{-m}$.

\subsubsection{Expected autocorrelogram under the homogeneous Poisson model}

For a homogeneous Poisson process with rate $r$, disjoint bins are independent
and
\[
\mathbb{E}[x_i]\approx r\Delta t,
\qquad
\mathbb{E}[x_i x_{i+m}]
\approx
(r\Delta t)^2
...
(M-|m|)r^2\Delta t^2
=
r^2(T-|m|\Delta t)\Delta t.
\]
Substituting this into the edge-corrected estimate gives
\[
\mathbb{E}\!\left[
\frac{N_m}{(T-|m|\Delta t)\Delta t}
-r^2
...
\qquad m\ne 0.
\]
So, after baseline subtraction, a homogeneous Poisson spike train should have
no systematic nonzero-lag autocorrelation.

Conditioning on the observed spike count $n$ gives a closely related empirical
baseline. Given $n$ spikes uniformly scattered through the recording, the
conditional probability that a second spike falls in a particular lag bin is
approximately $(n-1)\Delta t/T$. Thus the nonzero-lag raw pair count has
expectation approximately
\[
\mathbb{E}[N_m]\approx n\,\frac{n-1}{T}\Delta t,
\qquad 0<|m|\ll M.
\]
The factor $(n-1)\Delta t/T$ is the conditional probability that, given one
spike, another spike falls in the bin at lag $m$. This form is useful because
it uses the observed count $n$ rather than the unknown rate $r$. Thus the
normalized histogram
\[
H_m:=
\frac{1}{T}
\left(
N_m-\frac{n(n-1)}{T}\Delta t
\right)
\]
has expectation near zero for nonzero lags under the homogeneous Poisson baseline.

In the continuous limit the centered Poisson autocorrelation is concentrated entirely at zero lag:
\[
Q_{\rho\rho}(\tau)=\langle r\rangle\delta(\tau).
\]
This statement is the limit of the binned calculation. For nonzero lag,
independence gives $\mathbb{E}[\rho(t)\rho(t+\tau)]=r^2$, and centering removes
that baseline. At zero lag, however, every spike is perfectly coincident with
itself. As $\Delta t\to 0$, the zero-lag bin becomes narrower and taller while
its area remains $r$, producing the delta function.

\subsection{Interpreting autocorrelograms}

The lecture examples emphasize three recurring motifs.

\paragraph{Refractory trough.}
A deficit of pairs near $\tau=0$ reflects the fact that a neuron cannot fire again immediately after a spike. This is the autocorrelation version of the short-ISI gap.

\paragraph{Bursting peak.}
An excess of pairs at short positive and negative lags indicates that spikes tend to come in clusters. If the dominant within-burst interval is about $5\,\mathrm{ms}$, the autocorrelogram often has side structure at that scale.

\paragraph{Oscillatory structure.}
Repeated peaks at regular lag intervals indicate rhythmic modulation. A $40\,\mathrm{Hz}$ rhythm has period
\[
\frac{1}{40\,\mathrm{Hz}}=25\,\mathrm{ms},
\]
so correlogram peaks separated by about $25\,\mathrm{ms}$ suggest gamma-frequency coordination.

These motifs are useful for cell classification. For example, principal cells and interneurons can differ in firing rate, burst structure, refractory signatures, and autocorrelogram shape. The important lesson is that such distinctions are not contained in the mean rate alone.

\subsection{Cross-correlograms and functional connectivity}

For two distinct spike trains, define binary binned responses $x_i$ and $y_i$. The raw cross-correlation pair count is
\[
N_m^{xy}:=\sum_{i=1}^{M} x_i y_{i+m}.
\]
Unlike autocorrelation, there is no self-pair contribution, so $m=0$ is not automatically special. Under independent homogeneous Poisson spiking, the expected baseline is approximately
\[
\mathbb{E}[N_m^{xy}]
\approx
\frac{nn'}{T}\Delta t,
\]
where $n$ and $n'$ are the two spike counts.

The lag sign matters. With the convention above, a peak at positive $m$ means that spikes in $y$ tend to occur after spikes in $x$ by $m\Delta t$. If the peak is narrow and short-latency, it is consistent with a fast excitatory influence from the neuron represented by $x$ to the neuron represented by $y$. A narrow trough shortly after zero is consistent with inhibition. These interpretations are hypotheses, not proofs: common input, oscillatory phase-locking, firing-rate co-modulation, finite sample effects, and spike-sorting errors can produce correlogram structure without a direct synapse.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/aand-lecture3-correlogram-connectivity.png}
\caption{Cross-correlogram intuition. A short-latency peak to the right of zero suggests that spikes in cell A are followed by excess spikes in cell B; a short-latency trough suggests suppressed probability. Such motifs are evidence for functional relationships, but anatomical connectivity requires additional controls and interpretation.}
\end{figure}

\subsection{Why correlograms matter for microcircuits}

The broader motivation is functional connectivity in local circuits. In structures such as entorhinal cortex, hippocampus, and sensory cortex, computation depends not only on which neurons fire but also on how their spikes are coordinated. Spike-train correlations therefore serve as a bridge between raw extracellular recordings and circuit-level hypotheses:
\begin{itemize}
    \item autocorrelograms characterize the temporal organization of individual neurons,
    \item cross-correlograms characterize lagged relationships between pairs of neurons,
    \item and deviations from Poisson baselines identify structure that a rate-only model has failed to explain.
\end{itemize}

The lecture's examples from visual cortex and entorhinal microcircuits should be read in this spirit. Correlograms do not replace mechanistic experiments. They provide a statistically disciplined screen for temporal structure, candidate cell classes, and candidate interactions that deserve closer analysis.

\subsection{Conceptual takeaways from Lecture 3}

\begin{itemize}
    \item The Poisson process is a baseline model: it predicts $F=1$, $\mathrm{CV}=1$, exponential ISIs, and no centered nonzero-lag correlations.
    \item Refractory periods, bursting, oscillations, and shared network states are visible as structured deviations from that baseline.
    \item The inhomogeneous Poisson model keeps the independent-spiking assumption but allows the rate $\lambda(t)$ to vary over time.
    \item Autocorrelograms compare all spike pairs within one train and therefore generalize ISI statistics.
    \item Cross-correlograms compare spike pairs across neurons and are central tools for detecting possible functional connectivity.
    \item Normalization and baseline subtraction matter: raw pair histograms are intuitive, but centered rate-density correlations are the mathematically interpretable objects.
\end{itemize}

\subsection*{Sources mentioned in Lecture 3}

\begin{itemize}
    \item P. Dayan and L. F. Abbott (2001), \emph{Theoretical Neuroscience: Computational and Mathematical Modeling of Neural Systems}. MIT Press. Lecture 3 draws especially on Chapter 1.4.
    \item G. Buzs\'aki (2004), in-vivo identification of neurons and connections discussed in the lecture slides.
    \item J. J. Jun et al. (2017), Neuropixels high-density silicon probes discussed in the lecture slides.
    \item K. D. Harris et al. (2000), autocorrelogram examples for refractory periods and bursting.
    \item Quilichini, Sirota, and Buzs\'aki (2010), correlogram examples for cell classification and functional connectivity.
    \item J. J. Tukker et al. (2022), entorhinal-cortex microcircuit context discussed in the lecture slides.
\end{itemize}

\clearpage
\input{lectures/spikes-part-1-lecture-4}

\clearpage
\input{lectures/spikes-part-1-lecture-5}
...
\section{Mathematical Appendix: Correlation, Autocorrelation, and Convolution}

Lecture 2 used correlation functions in a finite observation window,
\[
Q_{xy}(\tau):=\frac{1}{T}\int_0^T x(t)\,y(t+\tau)\,dt,
\]
because this is the version that appears naturally in spike-triggered averaging. For mathematical intuition, it is often helpful to temporarily suppress the window factor and think in terms of the infinite-domain formulas. The lecture's definition is then just the windowed and rescaled version of the same idea.

\subsection{Correlation as a lagged comparison}

The cross-correlation asks a simple question: if one signal is shifted by $\tau$, how much does it overlap with the other? Large positive values indicate that peaks in $x$ tend to line up with peaks in the shifted $y$; negative values indicate that peaks in one signal line up with troughs in the other.

For discrete data, the same idea becomes
\[
Q_{xy}[m] = \frac{1}{N}\sum_{k=0}^{N-1} x[k]\,y[k+m].
\]
So correlation is not a mysterious new operation. It is just a sliding inner product. Each lag $\tau$ or index shift $m$ produces one number, and the full correlation function records how that overlap changes with the shift.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/aand-appendix-correlation-convolution.png}
\caption{Appendix view of the three related operations. Cross-correlation compares two signals at different lags, autocorrelation compares a signal with itself, and convolution uses a shifted kernel to build a new output rather than a similarity score.}
\end{figure}

\subsection{Autocorrelation}

Setting $y=x$ gives the autocorrelation,
\[
Q_{xx}(\tau)=\frac{1}{T}\int_0^T x(t)\,x(t+\tau)\,dt.
\]
...
\[
Q_{xx}(-\tau)=\frac{1}{T}\int_0^T x(t)\,x(t-\tau)\,dt
=\frac{1}{T}\int_0^T x(u+\tau)\,x(u)\,du
=Q_{xx}(\tau),
\]
up to the same boundary approximation already made in the lecture when $|\tau|\ll T$.

\paragraph{Zero-lag value.}
At $\tau=0$,
\[
Q_{xx}(0)=\frac{1}{T}\int_0^T x(t)^2\,dt \ge 0,
\]
which is the average signal power or mean squared amplitude. This is why autocorrelations typically peak at zero lag.

\paragraph{Bounded size.}
The Cauchy--Schwarz inequality gives
\[
|Q_{xy}(\tau)| \le \sqrt{Q_{xx}(0)\,Q_{yy}(0)}.
\]
So a correlation can never exceed the geometric mean of the two zero-lag powers. This is the continuous-time analog of the usual bound on an inner product.

In Lecture 2, the most important autocorrelation was the stimulus autocorrelation,
\[
Q_{ss}(\tau)=\frac{1}{T}\int_0^T s(t)\,s(t+\tau)\,dt.
\]
When $Q_{ss}$ is sharply concentrated near $\tau=0$, the stimulus values at different times are nearly independent. When $Q_{ss}$ is broad, nearby time points are statistically coupled, and reverse-correlation estimates become blurred across neighboring lags.

\subsection{Convolution}

Convolution is defined by
\[
(h*s)(t)=\int_{-\infty}^{\infty} h(t-\tau)\,s(\tau)\,d\tau.
\]
This looks similar to correlation, but the interpretation is different. Correlation produces a \emph{similarity as a function of lag}; convolution produces a \emph{new signal as a function of time}. The kernel $h$ is flipped, shifted to the evaluation time $t$, weighted by the input values, and integrated.

It helps to remember the sign difference:
\begin{itemize}
    \item correlation keeps the second signal in the form $y(t+\tau)$ and asks ``how similar are these two waveforms if I slide one of them?'';
    \item convolution uses $h(t-\tau)$ and asks ``how much should each shifted copy of the kernel contribute to the output at time $t$?''
\end{itemize}

There is also a precise algebraic relation between the two operations. If $\tilde{y}(t):=y(-t)$ is the time-reversed signal, then
\[
Q_{xy}(\tau)=\frac{1}{T}(x * \tilde{y})(-\tau).
\]
So correlation is just convolution with one signal time-reversed, followed by a sign change in the lag argument.

\subsection{Special cases worth memorizing}

Several identities recur throughout systems neuroscience:
\begin{align*}
\delta * s &= s,\\
\delta(\,\cdot - t_0\,) * s &= s(t-t_0),\\
Q_{xx}(0) &= \frac{1}{T}\int_0^T x(t)^2\,dt.
\end{align*}
The first identity says the delta function acts as the identity element for convolution. The second says a shifted delta picks out a shifted copy of the signal. The third says the zero-lag autocorrelation is signal power.

These small formulas already explain much of the lecture:
\begin{itemize}
    \item white noise is useful because its autocorrelation is approximately delta-shaped,
    \item the STA is a reverse correlation because the lag enters with a minus sign,
    \item and firing-rate estimates built from kernels are convolutions of a spike train with a smoothing filter.
\end{itemize}

\subsection{Frequency-domain viewpoint}

If one introduces the Fourier transform
\[
X(\omega)=\int_{-\infty}^{\infty} x(t)e^{-i\omega t}\,dt,
\]
then convolution in time becomes multiplication in frequency:
\[
\mathcal{F}\{h*s\}(\omega)=H(\omega)S(\omega).
\]
This is why filters are often described by which frequencies they pass or suppress.

Autocorrelation also has a spectral interpretation. Under the usual conventions, the Fourier transform of the autocorrelation is proportional to the power spectrum:
\[
\mathcal{F}\{Q_{xx}\}(\omega)\propto |X(\omega)|^2.
\]
So a narrow autocorrelation corresponds to a broad frequency spectrum, whereas a broad autocorrelation corresponds to concentration in low frequencies. This is another way to understand why temporally smooth stimuli do not probe all lags equally well.

\section{Mathematical Appendix: Linear Time-Invariant Systems, Delta Functions, and Generating Responses}

The filtering formulas in Lecture 1 and the reverse-correlation formulas in Lecture 2 both rely on the same systems idea: once one knows how a linear time-invariant system responds to a single impulse, one can generate its response to any input.

\subsection{Linearity and time invariance}

Let $\mathcal{L}$ denote a system that maps an input signal $x$ to an output signal $y=\mathcal{L}[x]$.

\paragraph{Linearity.}
For any constants $a,b$ and inputs $x_1,x_2$,
\[
\mathcal{L}[a x_1 + b x_2] = a\,\mathcal{L}[x_1] + b\,\mathcal{L}[x_2].
\]
This says weighted sums can be pushed through the system.

\paragraph{Time invariance.}
If $y(t)=\mathcal{L}[x](t)$, then shifting the input by $t_0$ shifts the output by the same amount:
\[
\mathcal{L}[x(\,\cdot - t_0\,)](t)=y(t-t_0).
\]
So the system's rule does not depend on absolute clock time. It reacts the same way today as it would tomorrow, provided the input shape is the same up to a shift.

\subsection{The delta function as an idealized elementary input}

The Dirac delta is not an ordinary function but a distribution characterized by the sifting identity
\[
\int_{-\infty}^{\infty} \delta(t-\tau)\,f(\tau)\,d\tau = f(t).
\]
This makes $\delta(t-\tau)$ the idealized ``elementary input at time $\tau$''.

Any signal can be decomposed into such elementary inputs:
\[
x(t)=\int_{-\infty}^{\infty} x(\tau)\,\delta(t-\tau)\,d\tau.
\]
This equation is exact in the distributional sense. In discrete form it becomes the Riemann-sum intuition
\[
x(t)\approx \sum_k x(t_k)\,\Delta t\,\delta(t-t_k),
\]
which is the picture used in the appendix figure below.

\subsection{Impulse response and the convolution formula}

Define the \emph{impulse response} of the system by
\[
h(t):=\mathcal{L}[\delta](t).
\]
By time invariance, the response to a delta at time $\tau$ is a shifted impulse response:
\[
\mathcal{L}[\delta(\,\cdot-\tau\,)](t)=h(t-\tau).
\]
By linearity, a scaled delta $a\,\delta(\,\cdot-\tau\,)$ produces a scaled output $a\,h(t-\tau)$.

Now insert the delta decomposition of the input:
\begin{align*}
y(t)
&=\mathcal{L}[x](t)\\
&=\mathcal{L}\!\left[\int_{-\infty}^{\infty} x(\tau)\,\delta(\,\cdot-\tau\,)\,d\tau\right](t)\\
&=\int_{-\infty}^{\infty} x(\tau)\,\mathcal{L}[\delta(\,\cdot-\tau\,)](t)\,d\tau\\
&=\int_{-\infty}^{\infty} x(\tau)\,h(t-\tau)\,d\tau.
\end{align*}
This is the convolution formula. The entire input-output behavior of an LTI system is encoded in a single function $h$.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/aand-appendix-lti-impulse-response.png}
\caption{Generating the response of a linear time-invariant system. Decompose the input into weighted shifted deltas, replace each delta by the correspondingly shifted impulse response, and then add all elementary responses to obtain the convolution integral.}
\end{figure}

\subsection{Why this matters for spike trains}

This abstract derivation reproduces one of the key lecture formulas immediately. If the input is a spike train,
\[
\rho(t)=\sum_i \delta(t-t_i),
\]
then convolution with a kernel $w$ gives
\[
(w*\rho)(t)=\int_{-\infty}^{\infty} w(t-\tau)\rho(\tau)\,d\tau
=\sum_i w(t-t_i).
\]
So the rate-estimation formula from Lecture 1 is exactly the response of an LTI filter whose impulse response is $w$ to a spike-train input made of deltas.

The same logic explains synaptic and membrane filtering. If each presynaptic spike evokes a stereotyped postsynaptic current shape $h(t)$, then a train of spikes produces a sum of shifted copies of that shape. In other words, the ``one kernel per spike'' picture is not merely a convenient smoothing trick; it is the natural output of an LTI system driven by point events.

\subsection{Causality and admissible kernels}

Many physical systems are \emph{causal}, meaning the output at time $t$ cannot depend on future input values. For an LTI system, causality is equivalent to
\[
h(t)=0 \qquad \text{for } t<0.
\]
This is why the alpha kernel from Lecture 1 is conceptually attractive: it can be interpreted as the impulse response of a causal filter. By contrast, a Gaussian smoothing kernel is not causal because it extends to negative times, so it uses future spikes to estimate the present rate.

\subsection{Discrete-time version}

Exactly the same logic holds for sampled data. If $x[n]$ is a discrete signal and $h[n]$ is the discrete impulse response, then
\[
y[n]=\sum_{k=-\infty}^{\infty} h[n-k]\,x[k].
\]
This is the formula implemented in most numerical filters. The continuous integral becomes a sum, but the interpretation is unchanged: shifted weighted copies of $h$ are added to build the output.

\subsection{Conceptual summary}

The delta function is useful because it isolates an elementary event at a precise time. LTI systems are useful because they reduce the full input-output map to one impulse response. Convolution is the bridge between them: it says that an arbitrary input can be assembled from deltas, and the corresponding output can be assembled from shifted impulse responses. This viewpoint links several parts of the course that might otherwise seem separate:
\begin{itemize}
    \item spike trains are sums of deltas,
    \item firing-rate estimates are convolutions with smoothing kernels,
    \item white-noise analysis relies on delta-like autocorrelations,
    \item and linear receptive-field models are specified completely by their kernels.
\end{itemize}
